...
test('a road answers with its narrowest matching rule, not its widest', () => {
  // Rule order is match priority and Chunghwa Post writes exceptions before
  // the '全' catch-all, so any order the bundle imposes of its own -- see
  // scripts/pack.py -- silently answers with the wrong one of two rules that
  // both match. Every case here has a wider rule that would win instead.
  assert.equal(find('新北市三峽區三樹路291號'), '237641');  // vs 全 -> 237012
  assert.equal(find('新北市三峽區三樹路100號'), '237012');  // no rule but 全
  assert.equal(find('新北市貢寮區巫里岸街2號'), '228063');  // vs 全 -> 228003
  // 單561號至579號 vs 單159號至675號: both start '單', neither is the catch-all.
  assert.equal(find('宜蘭縣羅東鎮公正路561號'), '265607');
});
...
test('never contradicts the Python reference', () => {
  ...
  //
  // The reference is patched at fixture-build time to read each road's rules
  // in CSV order (scripts/build_golden_fixture.py) -- unpatched it answers 459
  // of these queries from the wrong rule. Chunghwa Post's data is the ground
  // truth here, not upstream's reading of it.
  //
